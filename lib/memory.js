...
export class MemorySystem {
  ...
  async loadUserProfile() {
    try {
      const data = await storageGet({ userProfile: null });
  ...
  async saveUserProfile() {
    await storageSet({ userProfile: this.userProfile });
  }
...
只返回最相关的条目编号，用逗号分隔，如：0,3,1,5,2`
  ...
  async autoSaveIfWorth(question, answer, pageContext, aiClient) {
    ...
    try {
      ...
      const result = await this.kb.saveEntry({
        ...
        language: pageContext?.language || 'other'
      });

      // saveEntry 内部已做查重，如果重复会返回 {duplicate: true}
  ...
  async toPrompt(query, aiClient = null) {
    try {
      ...
      if (this.userProfile?.level) {
        ...
        if (Array.isArray(this.userProfile.languages) && this.userProfile.languages.length > 0) {
      ...
      console.warn('[PageWise] memory.toPrompt error (gracefully degraded):', e);
      return '';
    }
  }
}
