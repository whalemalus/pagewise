 ...
 *
 * 提供搜索记录管理功能:
 *   - addSearchQuery(query)            — 保存搜索记录（含时间戳）
 *   - getSearchHistory(limit)          — 获取最近搜索记录
 *   - getPopularSearches(limit)        — 获取最热门搜索
 *   - getSearchSuggestions(partial)    — 根据前缀返回自动补全建议
 *   - clearHistory()                   — 清除所有历史
 *   - removeSearchEntry(id)            — 删除指定搜索记录
 *
 * 设计约束:
...
export function removeSearchEntry(id) {
  ...
  const [removed] = _history.splice(idx, 1)
  _queryIndex.delete(removed.query)
  return true
}
