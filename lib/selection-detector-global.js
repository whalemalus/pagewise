/**
 * SelectionDetector — Non-module version for content scripts
 * Original: lib/selection-detector.js (ES Module)
 * This file registers SelectionDetector as a global for IIFE content scripts.
 */
(function() {
'use strict';
...
class SelectionDetector {
  ...
  _matchRule(rule, text) {
    if (rule.minLength && text.length < rule.minLength) return null;
    if (rule.pattern && rule.pattern.test(text)) {
      const result = { type: rule.type, confidence: rule.confidence };
      if (rule.detectLanguage) result.language = rule.detectLanguage(text);
      return result;
    }
    if (rule.patterns) {
  ...
  detectBatch(texts) { return texts.map(t => this.detectType(t)); }
  getSupportedTypes() { return ['code', 'url', 'error', 'math', 'english', 'unknown']; }
}

globalThis.SelectionDetector = SelectionDetector;
})();
