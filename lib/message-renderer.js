...
export class MessageRenderer {
  ...
  /**
   * 在 AI 消息 DOM 构建完成后，扫描行内 <code> 和 <blockquote> 元素，
   * 注入 data-quote 属性并添加可点击样式，绑定 locateAndHighlight 消息。
   * @param {HTMLElement} messageDiv - AI 消息 DOM 元素
   */
  _injectQuoteAttributes(messageDiv) {
    // 行内代码 — 排除 code block 内的 code（pre code 跳过）
  ...
  /**
   * 发送 locateAndHighlight 消息到 content script
   * @param {string} text - 要定位的文本
   */
  async _sendLocateAndHighlight(text) {
    if (!text) return;
    try {
      const result = await chrome.tabs.sendMessage(this.currentTabId, {
  ...
  /**
   * 重置渲染器：清空所有消息并重建 sentinel 元素。
   * 用于清除对话时调用，避免直接 innerHTML='' 导致 sentinel 被删除。
   */
  reset() {
    // 1. 断开旧的 observer
    if (this._observer) {
