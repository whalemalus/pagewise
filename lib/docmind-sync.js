...
export class DocMindSyncManager {
   ...
   * @param {Object} [options.graph] - 图谱数据（JSON-LD 格式或 { entities, relations }）
   * @returns {Promise<{status: string, knowledge: Object, bookmarks: Object, graph: Object}>}
   */
  ...
  /**
   * 执行图谱双向同步：推送本地变更到 DocMind，拉取远程变更到本地
   *
   * @param {Object} localGraphData - 本地图谱数据（JSON-LD 格式）
   * @returns {Promise<{sent: Object, received: Object, status: string}>}
   */
  ...
  /**
   * 获取默认的 storage.get 函数（自动降级 sync → local）
   */
  ...
  /**
   * 获取默认的 storage.set 函数（自动降级 sync → local）
   */
