...
export class KnowledgeBase {
  constructor() {
    ...
    this._indexWordsById = new Map();  // Map<id, Set<word>> — ID-only，不持有完整条目
    // N-gram 索引（惰性构建，优化子串搜索）
    this._ngramIndex = null;           // Map<ngram, Set<entry_id>>
    this._ngramSize = 3;
    // 条目总数缓存
  ...
  /**
   * 索引预热 — 主动构建倒排索引和 N-gram 索引
   * 在 init() 后调用，避免首次搜索时的惰性构建延迟
  ...
  /**
   * 清空搜索和统计缓存（在数据变更时调用）
   * 注意: 不清除 _indexWordsById — 当索引已构建时，_addToIndex/_removeFromIndex
   * 已增量维护索引，无需重建
   */
  _invalidateCaches() {
  ...
  /**
   * 获取条目总数（带缓存）
   * 使用 IDBObjectStore.count() 避免加载全部记录到内存
   * @returns {Promise<number>} - 条目总数
   */
  async getTotalCount() {
    await this.ensureInit();
  ...
  /**
   * 分页获取条目（按 createdAt 倒序）
   * @param {Object} options - 分页选项
   * @param {number} [options.page=1] - 页码（从 1 开始）
   * @param {number} [options.pageSize=10] - 每页条目数
  ...
  _addToIndex(entry) {
    ...
    // 仅存储 ID → 词集合映射，不持有完整条目对象（AC-3: 内存可控）
  ...
  _removeFromIndex(id) {
    ...
    // 从 N-gram 索引移除
    if (this._ngramIndex) {
      // 需要从 IDB 获取条目来计算 ngram（内存中不持有完整条目）
      // 使用 entries 数量遍历 ngram 索引中的引用
  ...
  /**
   * 根据 ID 集合从 IndexedDB 批量获取条目
   * 优化: 使用 store.get(id) 逐个获取，避免全量 cursor 扫描
   * @param {Set<number|string>} ids - 条目 ID 集合
   * @returns {Promise<Map<number|string, Object>>} - id → entry 映射
   */
  ...
  /**
   * 使用 includes() 精确过滤条目数组
   * @param {Array<Object>} entries - 条目数组
   * @param {string} lowerQuery - 小写查询字符串
   * @returns {Array<Object>} - 匹配的条目
   */
  _filterEntries(entries, lowerQuery) {
    return entries.filter(entry => this._matchesEntry(lowerQuery, entry));
  }

  /**
   * 从索引中获取所有条目（从 IDB 按需读取，不持有完整副本）
   * @returns {Promise<Array<Object>>} - 所有条目
   */
  ...
  /**
   * 高效获取聚合统计（标签、分类、语言）
   * 使用索引优先，不加载完整条目；结果缓存，数据变更时失效
   * @returns {Promise<{tags: Object, categories: Object, languages: Object}>}
   */
  ...
  /**
   * 高效键游标分页（避免 O(offset) 跳过）
   * 使用上一页最后条目的 createdAt 作为游标键
   * @param {Object} options - 分页选项
   * @param {number} [options.pageSize=10] - 每页条目数
   * @param {string|null} [options.lastCreatedAt=null] - 上一页最后条目的 createdAt
   * @param {number|null} [options.lastId=null] - 上一页最后条目的 ID（用于去重）
   * @returns {Promise<{entries: Array, total: number, hasMore: boolean}>}
   */
  async getEntriesPagedByKey({ pageSize = 10, lastCreatedAt = null, lastId = null } = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let request;
      if (lastCreatedAt) {
        // 使用 IDBKeyRange 打开 <= lastCreatedAt 的范围，prev 方向
        const range = IDBKeyRange.upperBound(lastCreatedAt, false);
      ...
      request.onsuccess = (event) => {
        ...
        // 跳过上一页最后条目本身（与 lastCreatedAt 相同且 lastId 匹配）
        if (lastCreatedAt && lastId && !skipped) {
          if (cursor.value.createdAt === lastCreatedAt && cursor.value.id === lastId) {
            cursor.continue();
            return;
          }
          skipped = true;
  ...
  /**
   * 带过滤条件的键游标分页
   * 仅标签过滤时使用 IDB multiEntry 索引；语言过滤回退客户端过滤
   * @param {Object} options - 分页选项
   * @param {number} [options.pageSize=10] - 每页条目数
   * @param {string|null} [options.tag=null] - 标签过滤
   * @param {string|null} [options.language=null] - 语言过滤
   * @param {string|null} [options.lastCreatedAt=null] - 上一页最后条目的 createdAt
   * @param {number|null} [options.lastId=null] - 上一页最后条目的 ID
   * @returns {Promise<{entries: Array, total: number, hasMore: boolean}>}
   */
  async getEntriesPagedByKeyFiltered({ pageSize = 10, tag = null, language = null, lastCreatedAt = null, lastId = null } = {}) {
    ...
    if (tag) {
      return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
          ...
          // 语言过滤（客户端过滤，IDB 无语言索引）
          if (language) {
            entries = entries.filter(e => (e.language || 'other') === language);
          }
          ...
          if (lastCreatedAt && lastId) {
            for (let i = 0; i < entries.length; i++) {
              if (entries[i].createdAt === lastCreatedAt && entries[i].id === lastId) {
                startIndex = i + 1;
                break;
              }
            }
    ...
    // 语言过滤但无标签 — 无 IDB 索引，回退全量加载 + 客户端过滤
    if (language) {
      const allEntries = await this.getAllEntries(10000);
  ...
  /**
   * 使用 N-gram 索引查找子串匹配的候选条目 ID
   * @param {string} query - 小写查询
   * @returns {Set<number|string>} - 候选条目 ID 集合
   */
  ...
  /**
   * 全文搜索（基于倒排索引 + N-gram 索引加速，短查询回退全量扫描）
   */
  async search(query) {
    await this.ensureInit();
    ...
    // Phase 3: 若精确词和子串都无结果，使用 N-gram 索引
    if (candidateIds.size === 0) {
  ...
  async getAllTags() {
    ...
    // 优先使用索引中的条目（从 IDB 按需读取），避免持有完整副本
    const allEntries = this._indexBuilt
      ? await this._getAllEntriesFromIndex()
      : await this.getAllEntries(10000);
    const tagCount = {};
  ...
  /**
   * 获取统计信息
   * 优化: 使用 getTotalCount() 避免加载全部条目，仅加载最近 5 条
   */
  async getStats() {
    await this.ensureInit();
  ...
  /**
   * 查找与指定条目相关的条目
   * 优化: 使用索引缩小候选范围，再从 IDB 批量获取并计算相似度
  ...
  async findRelatedEntries(entryId, limit = 5) {
    ...
    let candidates;
    if (this._indexBuilt) {
      const targetWords = this._extractWords(targetEntry);
      const candidateIds = new Set();
      for (const word of new Set(targetWords)) {
        const ids = this._searchIndex.get(word);
        if (ids) {
          for (const id of ids) {
            if (id !== entryId) candidateIds.add(id);
          }
        }
      }
      if (candidateIds.size > 0) {
        ...
        const allEntries = await this.getAllEntries(10000);
        candidates = allEntries.filter(e => e.id !== entryId);
      }
    } else {
      const allEntries = await this.getAllEntries(10000);
      candidates = allEntries.filter(e => e.id !== entryId);
  ...
  /**
   * 综合搜索 — 先关键词匹配（使用倒排索引加速），再语义排序，合并去重
   * @param {string} query - 搜索查询
   * @param {number} limit - 返回数量上限
