...
export function parseSimpleYaml(yaml) {
  ...
  for (const line of lines) {
    ...
    if (indent === 0) {
      ...
      if (currentObject !== null && currentKey) {
      ...
      if (trimmed.startsWith('- ')) {
        ...
        // Check if array item is an object (has colon)
        if (itemValue.includes(': ') && !itemValue.startsWith('"') && !itemValue.startsWith("'")) {
          // Start of an object in the array
          if (currentObject === null) {
            currentObject = {}
          }
          // Check if this is a new object (same indent as array items)
          if (indent === blockIndent && Object.keys(currentObject).length > 0) {
        ...
        if (currentArray !== null && currentObject === null) {
          // This is inside an array item object context
        }
        const colonIdx = trimmed.indexOf(':')
